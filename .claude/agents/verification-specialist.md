...
Read `docs/architecture/project.md` before doing anything else. Extract:
...
```bash
BACKEND_DIR=$(grep BACKEND_DIR docs/architecture/project.md | cut -d'|' -f3 | xargs)
...
```bash
FRONTEND_DIR=$(grep FRONTEND_DIR docs/architecture/project.md | cut -d'|' -f3 | xargs)
...
- If server can start: curl or fetch key endpoints, verify response shape matches the project's error contract (read from `docs/architecture/openapi.yaml` or equivalent)
...
- Cross-reference with API spec file (path in `project.md` or `docs/architecture/openapi.yaml`)
...
1. Read `docs/architecture/project.md` for stack, build commands, and conventions.
